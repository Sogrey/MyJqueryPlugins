...
var jqueryAlert = function(opts) {
    ...
    var dialog = {};
    dialog.time = 450;
    dialog.init = function() { dialog.framework() };
    ...
    if (isHaveTouch) { dialog.event = "click" } else { dialog.event = "touchstart" }
    ...
    } else { $content.html(option.content) }
    dialog.framework = function() {
        dialog.buttons = [];
        for (var key in option.buttons) { dialog.buttons.push(key) }
        dialog.buttonsLength = dialog.buttons.length;
        $container.append($title).append($content);
        if (option.style == "pc") { $container.append($closeBtn).addClass("pcAlert") }
        ...
        if (option.width == "auto") { $container.css("width", $container[0].clientWidth + 10) }
        ...
        for (var key in option.buttons) {
            ...
            if (option.style != "pc") { $button.css({ "width": Math.floor(($container[0].clientWidth) / dialog.buttonsLength), }) }
        ...
        if (dialog.buttonsLength > 0) {
            ...
            if (window.navigator.userAgent.indexOf("MSIE") >= 1) {
                if ($content[0].scrollHeight > $content[0].clientHeight) { $content.css("height", parseInt($content.css("height")) - 46) }
            }
        }
        ...
        $container.css({ "margin-left": -parseInt($container.css("width")) / 2, "margin-top": -parseInt($container.css("height")) / 2, });
        if (option.animateType == "scale") { $container.addClass("bounceIn") }
    ...
    dialog.toggleAnimate = function() {
        if (option.animateType == "scale") { return $container.removeClass("bounceIn").addClass("bounceOut") } else {
